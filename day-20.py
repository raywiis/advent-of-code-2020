...
def rot_once(body):
	...
	for col in range(len(body[0])):
		t_body += [((''.join([l[col] for l in body]))[::-1])]
	return t_body
...
def rotate(t, count):
	...
	for _ in range(count):
		...
		for col in range(len(n_body[0])):
			t_body += [((''.join([l[col] for l in n_body]))[::-1])]
		n_body = t_body
...
def grid_to_picture(grid):
	...
	for row_idx, row in enumerate(grid):
		tiles += [[]]
		for t in row:
			new_tile = [l[1:-1] for l in t[1][1:-1]]
	...
	for row_offset in range(len(picture) - monster_height):
		for col_offset in range(picture_len - monster_len):
			...
			for p_idx, p_line in enumerate(pat):
				if not loose_match(p_line, picture[row_offset + p_idx][col_offset:]):
					match = False
			if match:
				matches += 1
	return matches
